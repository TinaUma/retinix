...
## СТАТУС: композит ОПРОВЕРГНУТ бэктестом (2026-07)

Всё, что описано ниже, спроектировано как модель РИСКА. Бэктест на закрытых
задачах показал **AUC 0.4820** — различающей силы нет, это неотличимо от
подбрасывания монеты. Композит понижен до ОПИСАТЕЛЬНОГО: величина говорит,
насколько ТОНКО доказательство закрытия, и не предсказывает побег дефекта
(решение #212). Замер и метод: [`risk-model-backtest-2026-07.md`](risk-model-backtest-2026-07.md).

Этот раздел стоит ПЕРЕД ограничениями намеренно. Ограничения ниже перечисляют,
чем модель неточна; здесь сказано, что она не работает как задумано вообще, — и
читатель, открывший дизайн-документ вместо CHANGELOG, обязан узнать об этом до
того, как начнёт читать формулу.
