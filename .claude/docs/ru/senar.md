...
| Rule 2 | Определить границы задачи | `scope_write_gate`/`bash_write_gate` блокируют запись вне `scope_paths`; QG-0 жёстко блокирует старт medium/complex без scope |
| Rule 3 | Проверять по критериям | QG-0 + QG-2 совместный контроль |
| Rule 4 | Внешнее состязательное ревью | `risk_l3_trigger` блокирует закрытие с тонким доказательством до записи L3-ревью; ревьюер — сепаратный субагент `tausik-external-reviewer` |
| Rule 5 | Контрольный список верификации | Жёсткий гейт для тиров substantial/deep, чьи AC не ссылаются на существующий тест; ниже — эскалирующий warning |
...
> **О правилах 4–6.** С **v1.5** они применяются как жёсткие блокировки: **Rule 4** (внешнее состязательное ревью — сепаратный по модели, read-only субагент гейтит закрытия с тонким доказательством), **Rule 5** (контрольный список верификации — жёсткий гейт для планировочных тиров substantial/deep, совещательный ниже) и **Rule 6** (планирование отката — QG-0 блокирует новые medium/complex-задачи без задокументированного плана отката). Полная таблица enforcement — в [Матрице соответствия SENAR](senar-compliance-matrix.md).
