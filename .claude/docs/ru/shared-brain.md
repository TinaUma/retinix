...
**Локальное общее хранилище (1.8):** `~/.tausik-knowledge/knowledge.db` — не эта страница, а **[knowledge-store.md](knowledge-store.md)**. Оно НЕ уезжает наружу и потому НЕ редактируется; brain уезжает и потому редактируется. Разница между ними — единственное, что нужно держать в голове.
