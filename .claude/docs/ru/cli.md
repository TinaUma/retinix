...
## Вычеркивание из памяти (v1.9, решение #258)

Журнал задач append-only, у памяти нет `update`, у решений — только `list`. Это
сознательно: запись, которую можно молча переписать, перестаёт быть
доказательством. Но у неотзываемости обязан быть парный механизм, иначе первая
же ошибочно записанная строка становится вечной.

`redact` — этот механизм. Он есть **надпись поверх со следом**, а не удаление.

```bash
# Сухой прогон — ПО УМОЛЧАНИЮ. Ничего не пишет, показывает, что тронул бы.
tausik redact --pattern "internal.example.com" --label internal-host \
              --reason "адрес приватного хоста не уезжает в публичный репозиторий"

# Применить. НЕОБРАТИМО.
tausik redact --pattern "internal.example.com" --label internal-host \
              --reason "..." --apply

# Регулярным выражением вместо литерала
tausik redact --regex --pattern "alpha|beta|gamma" --label third-party-project --reason "..."

# Что было вычеркнуто в этом проекте
tausik redact list --limit 50
```

**Неверное регулярное выражение отвергается словами, а не трейсбеком.** `--regex` с непарсящимся шаблоном даёт именованный отказ с кодом 1: команда называет шаблон КАК НАБРАНО и приводит объяснение `re` вместе с позицией ошибки. Шаблон печатается без кавычек-repr намеренно — `re` сообщает о неполадке по ПОЗИЦИИ, а repr удваивает обратные слэши и сдвинул бы каждый индекс. Отказ происходит ДО чтения и записи, и он отличим от соседнего исхода «ноль совпадений»: непарсящийся шаблон и пустой результат — разные ответы с разными средствами.

**Что происходит с текстом.** Совпадение заменяется видимым маркером
`[вычеркнуто: <label>]`. Читатель обязан видеть, что здесь что-то стояло:
молча укороченная фраза неотличима от фразы, которая всегда была такой.

**Что происходит со следом.** На каждую тронутую колонку пишется строка в
таблицу `redactions`: сущность, поле, КЛАСС вычеркнутого, основание, число
замен, время. След не удаляется и не правится — он единственная причина, по
которой неотзываемому журналу можно верить после того, как переписывание стало
возможным.

**`--label` называет класс, а не значение.** След, цитирующий секрет, вернул бы
утечку в базу — в колонку, которую никто не догадается просканировать.

**Область действия.** Только колонки с прозой, объявленные одним местом —
`scripts/redact_scope.py`. Структурные колонки (`slug`, `status`, отметки
времени, внешние ключи) исключены: это адреса, а не текст, и переписывание
адреса ломает ссылающиеся строки, ничего написанного человеком не убирая.

**Восстановления нет.** Исходный текст не хранится нигде — ни в следе, ни в
теневой копии. Единственный путь назад — бэкап `.tausik/tausik.db`, снятый
ЗАРАНЕЕ; сухой прогон говорит об этом до того, как что-либо записано.

**После применения обязательно `tausik state export`** — проекция генерируется
из базы, и до пересборки дерево показывает старый текст.

**Ноль совпадений — отдельный исход, а не успех.** Команда говорит «no match»
своими словами: вызывающий, уверенный, что утечка есть, обязан узнать, что
ошибся образец, а не получить чистый выход наравне с настоящей работой.
