...
- **`scripts/brain_classifier.py`** (`classify()`) — оценивает **риск** явной публикации (маркеры, blocklist). С 1.8 назначение он не выбирает (решение #221).
...
| Инсайт из задачи, но может пригодиться вне репозитория | Сначала локально; обобщить формулировку перед выгрузкой в brain (`tausik brain move --to-brain <id>`, MCP, `tausik brain publish`). |

Если сомневаетесь — сначала **`tausik search`** / **`memory_search`** (и brain-поиск, если включён).

## Согласование с classifier

`classify()` отвечает только на вопрос, **насколько рискованной** выглядит явная публикация. Он **не** выбирает назначение и **не** сливает и не дедуплицирует строки. Сигналы «проектное» (пути, длинные slug, blocklist) → **высокий риск**, и публикация блокируется до подтверждения человеком — правило merge/new всё равно нужно **внутри** локальной памяти для качества FTS.

С 1.8 само ничего не маршрутизируется. Назначение выбираешь **ты**: проект по умолчанию, общее локальное хранилище через `--global`, Notion — только через `tausik brain move --to-brain <id>` (решение #221). Запись решения не публикует его никуда, а локальная копия безусловна (`service_decide.record`).
...
Hint **только подсказка** — не блокирует запись, не выбрасывает исключений, молчит когда совпадений нет. Детект идёт после успешной записи в `service_knowledge.memory_add`. Он также стоит в success-путях `brain_runtime.try_brain_write_decision` / `try_brain_write_web_cache`, но у `try_brain_write_decision` **с 1.8 нет ни одного вызывающего в проде** — решения сами в Notion не уходят (решение #221), так что этот путь достижим только из тестов.
