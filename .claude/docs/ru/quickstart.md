...
2. Сформулирует цель и критерии приёмки (что считается «готово») — **включая хотя бы
   один сценарий ошибки или граничный случай**. Этого требует Context Gate (QG-0), и
   агент добавит его за вас (например, «кнопка неактивна, пока не заполнен заголовок»).
   Писать его вручную не нужно — просто знайте, что агент его добавит.
...
> **Экономия вывода (`output_mode`, opt-in).** Ключ верхнего уровня `"output_mode": "off"` (по умолчанию) | `"caveman"`. Ортогонален `context_tier`: tier задаёт размер **входных** правил, `output_mode` жмёт **вывод** агента. При `caveman` bootstrap дописывает короткую директиву (по мотивам [скилла caveman](https://github.com/JuliusBrussee/caveman)): отвечать телеграфной прозой, но **код, команды, вывод инструментов и сообщения об ошибках — байт-в-байт**, а **AC-evidence, decisions и SPEC/ADAPT — полными** (их разбирают будущие агенты). Неверное значение молча откатывается в `off` — опечатка в knob'е экономии не должна ломать bootstrap. **На уже забутстрапленном проекте** учтите: `CLAUDE.md` / `AGENTS.md` / `.cursorrules` / `QWEN.md` и файл правил OpenCode сохраняются, если уже существуют, — переключение ключа их не перезапишет. Bootstrap скажет об этом прямо, а не сделает вид, что применил: удалите сгенерированный файл правил и перезапустите, либо вставьте директиву руками. Заявленные caveman «~65%» — это ИХ цифра, в среде TAUSIK не измерена. Если установлен и настоящий скилл caveman, `tausik doctor` сообщит о сосуществовании (его сжатие складывается с нашим) и предупредит о любом caveman-hook'е в `.claude/settings.json`, которым владеет TAUSIK.
...
`.tausik/` лежит в gitignore — база ваша. Коллегам нужны не файлы базы, а
*знания* проекта, и они едут в git текстом: задачи, решения и память
...
После того как забрали чужую работу, внесите её в свою базу:

```bash
git pull
tausik sync              # tausik/ → БД, файлы побеждают
```

Коллега, склонировавший репозиторий, получает историю проекта, а не пустую
базу, и никакого внешнего хранилища для этого держать не нужно.

`tausik status` сообщает, когда дерево и база расходятся. Он показывает
количества и называет **оба** направления — `tausik sync` (дерево → БД) и
`tausik state export` (БД → дерево), — не рекомендуя ни одного, потому что
количества доказывают, что стороны *различаются*, а не какая из них свежее.
Единственный однозначный сигнал — строки, которые есть в дереве и которых нет
в базе; именно его вы видите после `git pull`.

**[Полный контракт round-trip →](team-state-in-git.md)**
